...
import com.dto.GlobalConstants;
import com.opensymphony.xwork2.ActionSupport;
import com.service.FailedInfo;
import com.service.ServiceResult;
import com.service.SuccessData;
...
public abstract class BaseAction extends ActionSupport implements
		ServletRequestAware, ServletResponseAware, Constant {
	 ...
	 * @param msg
	 */
	protected void writeFailedResult(String errCode, String msg,
			String... target) {
		writeFailedResult(errCode, msg, target);
	}
	...
	protected void writeToJSON(JSON json) {
	 ...
	 */
	/*
	 * public static void writeFailedResult(HttpServletResponse response, String
	 * errCode, String msg, String...target){ try{ JSONObject jsonObj = new
	 * JSONObject(); jsonObj.put(ErrorCode.ERROR_CODE, errCode);
	 * jsonObj.put(ErrorCode.MSG, msg); if (target != null && target.length >
	 * 0){ jsonObj.put(ErrorCode.ERROR_TARGET, target[0]); } writeJSON(response,
	 * jsonObj); }catch(Exception ex){ log.error(null, ex); } }
	 */
	 ...
	 * @param failedInfo
	 */
	public static void writeFailedResult(HttpServletResponse response,
	 ...
	 * @param sd
	 */
	public static void writeSuccessData(HttpServletResponse response,
			SuccessData sd) {
		try {
			if (sd.getJSON() != null) {
	 ...
	 * @param sr
	 */
	public static void writeServiceReuslt(HttpServletResponse response,
			ServiceResult sr) {
		if (sr.isSuccess()) {
			if (sr.getJSON() != null) {
	...
	public static void writeToJSON(HttpServletResponse response, JSON json) {
		try {
			String strJson = json.toString();
			log.debug("writeJSON: " + strJson);
			response.setContentType("text/html;charset=UTF-8");
	...
	public static void writeToObject(HttpServletResponse response, Object obj) {
		try {
			String strJson = obj.toString();
			log.debug("writeJSON: " + strJson);
			response.setContentType("text/html;charset=UTF-8");
	...
	protected void writeToObject(Object obj) {
		writeToObject(response, obj);
	}
	...
	/*
	 * protected java.sql.Date toSQLDate(String d){ Date date; try { date =
	 * DateUtils.parseDate(d, GlobalConstants.DEF_DATEFORMAT); return new
	 * java.sql.Date(date.getTime()); } catch (ParseException e) { return null;
	 * }
	 * 
	 * }
	 */
	...
	/*
	 * protected java.sql.Timestamp toTimestamp(String d){ Date date; try { date
	 * = DateUtils.parseDateStrictly(d, GlobalConstants.DEF_DATETIME_FMT ,
	 * GlobalConstants.DEF_DATEFORMAT); return new Timestamp(date.getTime()); }
	 * catch (ParseException e) { return null; } }
	 * 
	 * protected java.sql.Time toSQLTime(String t){ Date time; try { time =
	 * DateUtils.parseDate(t, GlobalConstants.DEF_TIMEFORMAT); return new
	 * java.sql.Time(time.getTime()); } catch (ParseException e) { return null;
	 * } }
	 */
	...
	public void addActionError(String anErrorMessage) {
		...
		this.simpleJsonErrResult = ServiceResult.getFailedInstance("",
				anErrorMessage).getFailedInfo();
	}

	@Override
	public void addFieldError(String fieldName, String errorMessage) {
		super.addFieldError(fieldName, errorMessage);
		this.simpleJsonErrResult = ServiceResult.getFailedInstance(fieldName,
				errorMessage).getFailedInfo();
	}
